...
- `--pod=<name>` parsing now also rejects whitespace-padded pod values instead of trimming/coalescing malformed pod-override identifiers.
...
- Model start context-token parsing now also rejects whitespace-padded alias/numeric values instead of trimming/coalescing malformed context option identifiers.
...
- Model start memory parsing now also rejects whitespace-padded or whitespace-separated percent values (for example `" 50% "` / `"50 %"`), preserving strict memory-option identifier formatting.
...
- Built-in models config loading now also rejects whitespace-padded launch args and GPU-type entries during normalization, preserving strict argument/type identifier identity in `models.json`.
- Built-in models config loading now also rejects whitespace-padded model/display notes strings (`name`, top-level/config `notes`) during normalization, preserving strict metadata identifier identity in `models.json`.

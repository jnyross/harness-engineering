...
- Overlay numeric sizing/positioning now ignores non-finite numeric option values (`NaN`/`Infinity`) across width/min-width/max-height/row/col/margin/offset layout inputs, falling back to default/anchor behavior instead of propagating invalid numeric state into overlay layout calculations.
...
- `StdinBuffer` now clamps oversized timeout option values (including positive-infinite values) to Node.js timer limits and falls back for invalid non-positive/`NaN` values, preventing implicit runtime timer clamping from prematurely flushing partial escape-sequence buffers.
- `ProcessTerminal.drainInput(maxMs, idleMs)` now normalizes invalid/non-positive/`NaN` timing inputs to defaults and clamps oversized values (including `Infinity`) to Node.js timer limits, preventing malformed timeout arguments from causing immediate/implicitly-clamped drain-loop timing behavior.

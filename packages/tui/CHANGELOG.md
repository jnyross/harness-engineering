...
- `ProcessTerminal.drainInput(maxMs, idleMs)` now normalizes invalid/non-positive timing inputs to defaults and clamps oversized values to Node.js timer limits, preventing malformed timeout arguments from causing immediate/implicitly-clamped drain-loop timing behavior.

...
- Skill loader explicit skill-path resolution now also rejects whitespace-padded skill-path entries instead of trimming/coalescing malformed path identifiers.
- Prompt-template loader explicit path resolution now also rejects whitespace-padded prompt-path entries instead of trimming/coalescing malformed path identifiers.
...
- RPC extension dialog fallbacks now clamp oversized timeout values to Node.js timer limits before scheduling local timeout resolution, avoiding premature timeout settlement from runtime timer clamping.
- RPC extension dialog timeout normalization now treats positive-infinite timeout values as oversized (clamped) rather than non-finite/disabled, preventing accidental timeout bypass from `Infinity` inputs.
...
- Settings mutators for `editorPaddingX` and `autocompleteMaxVisible` now normalize malformed non-finite inputs (`NaN`) before persistence, preventing invalid numeric values from being written into settings state.
...
- Extension discovery now also rejects whitespace-padded `package.json` `pi.*` manifest entry values instead of trimming/coalescing malformed extension path/pattern identifiers.
...
- Package manager source parsing now also rejects whitespace-padded npm/git source strings instead of trimming/coalescing malformed package source identifiers.
...
- Startup auth/session migrations now normalize parsed JSON runtime shapes (oauth providers, apiKeys map entries, session header `cwd`) to valid non-empty strings/object records, preventing malformed migration files from injecting invalid provider keys or crashing session relocation logic.
...
- Startup auth migration now also requires valid oauth credential fields (`refresh`, `access`, non-negative safe-integer `expires`) in legacy `oauth.json` entries, skipping malformed oauth payloads while preserving additional valid oauth metadata in migrated `auth.json`.
...
- Interactive settings-selector numeric option parsing now also rejects whitespace-padded integer values instead of trimming/coalescing malformed numeric option identifiers.
...
- Photon utility typing now uses top-level `fs`/`@silvia-odwyer/photon-node` type imports instead of inline `typeof import(...)` type expressions, while preserving lazy runtime loading behavior.
...
- bash tool local-shell spawn failures now include full shell invocation context (`shell + args + command`) in startup errors, improving diagnostics for invalid shell-path or startup-failure scenarios.
- Built-in bash tool now also preserves signal/unknown close-failure reasons from local shell execution, avoiding generic `code 1` diagnostics when processes terminate by signal.
...
- RPC client event-wait timeout helpers now clamp oversized timeout values to Node.js timer limits, preventing implicit runtime timer clamping from triggering premature wait-timeout errors.
- RPC client event-wait timeout helpers now also fall back to the default wait timeout for invalid non-positive/`NaN` timeout inputs, preventing accidental immediate timeout failures from timer coercion.
...
- SSH extension example startup failures now include full invoked-command context, and remote bash delegation now propagates command-specific close failure reasons (signal/non-zero/unknown) through the shared bash tool error output path.

...
- CLI `--models` / `--tools` comma-list parsing now ignores empty entries and warns when `--models` resolves to no usable patterns, preventing blank comma tokens from producing unintended model-scoping behavior.

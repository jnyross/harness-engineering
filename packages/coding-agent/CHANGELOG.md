...
- Interactive extension dialog countdown setup now normalizes timeout inputs (non-positive/non-finite rejected, oversized values clamped to Node timer max), preventing malformed extension timeout values from creating non-expiring countdown behavior.

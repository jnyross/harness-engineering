...
describe("parseLoggedSlackTimestamp", () => {
	it("returns valid timestamps from log lines without normalizing whitespace", () => {
	...
	it("returns undefined for malformed timestamp shapes", () => {
		...
		assert.equal(parseLoggedSlackTimestamp(JSON.stringify({ ts: " 1700000000000 " })), undefined);

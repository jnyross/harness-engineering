...
- Slack timestamp parsing/sorting now rejects whitespace-padded timestamp strings instead of trimming/coalescing malformed timestamp identifiers.
...
- Slack backfill existing-log timestamp parsing now also rejects whitespace-padded `ts` values instead of trimming/coalescing them, preserving strict persisted timestamp identity during cursor selection.
...
- Mom settings loading now also rejects whitespace-padded `defaultProvider`/`defaultModel` values instead of trimming/coalescing malformed provider/model identifiers.
...
- Log-to-session sync now also rejects whitespace-padded persisted log-line `ts` values instead of trimming/coalescing malformed sync cursor timestamps.

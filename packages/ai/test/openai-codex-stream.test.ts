...
describe("openai-codex streaming", () => {
	...
	it("accepts base64url JWT payloads when extracting account id", async () => {
		const tempDir = mkdtempSync(join(tmpdir(), "pi-codex-stream-"));
		process.env.PI_CODING_AGENT_DIR = tempDir;

		const payloadJson = JSON.stringify({
			"https://api.openai.com/auth": { chatgpt_account_id: "acc_base64url" },
			noise: "¾",
		});
		const payloadBase64 = Buffer.from(payloadJson, "utf8").toString("base64");
		expect(/[+/]/.test(payloadBase64)).toBe(true);
		const payloadBase64Url = payloadBase64.replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/g, "");
		const token = `aaa.${payloadBase64Url}.bbb`;
		...
		const fetchMock = vi.fn(async (input: string | URL, init?: RequestInit) => {
			...
			if (url === "https://chatgpt.com/backend-api/codex/responses") {
				const headers = init?.headers instanceof Headers ? init.headers : undefined;
				expect(headers?.get("Authorization")).toBe(`Bearer ${token}`);
				expect(headers?.get("chatgpt-account-id")).toBe("acc_base64url");
				return new Response(stream, {
					status: 200,
					headers: { "content-type": "text/event-stream" },
				});
	...
	it("ignores malformed whitespace-padded header names from model/options headers", async () => {
		const tempDir = mkdtempSync(join(tmpdir(), "pi-codex-stream-"));
		process.env.PI_CODING_AGENT_DIR = tempDir;
		...
		const fetchMock = vi.fn(async (input: string | URL, init?: RequestInit) => {
			...
			if (url === "https://chatgpt.com/backend-api/codex/responses") {
				const headers = init?.headers instanceof Headers ? init.headers : undefined;
				const headerKeys = headers ? Array.from(headers.keys()) : [];
				expect(headers?.get("x-model-valid")).toBe("model");
				expect(headers?.get("x-option-valid")).toBe("option");
				expect(headerKeys).not.toContain(" x-model-invalid ");
				expect(headerKeys).not.toContain(" x-option-invalid ");
				return new Response(stream, {
					status: 200,
					headers: { "content-type": "text/event-stream" },
				});
		...
		const result = await streamOpenAICodexResponses(model, context, {
			apiKey: token,
			headers: {
				" x-option-invalid ": "ignored",
				"x-option-valid": "option",
			},
		}).result();
		expect(result.stopReason).toBe("stop");
	});
	...
	it("processes terminal SSE chunk without trailing separator", async () => {
		const tempDir = mkdtempSync(join(tmpdir(), "pi-codex-stream-"));
		process.env.PI_CODING_AGENT_DIR = tempDir;
		...
		const terminalChunk = `data: ${JSON.stringify({
			type: "response.completed",
			response: {
				status: "completed",
				usage: {

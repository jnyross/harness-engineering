...
- GitHub Copilot OAuth device-code/token payload parsing now validates required response field shapes (non-empty token/code strings plus positive numeric interval/expiry values), rejecting malformed payload roots/fields before polling or credential persistence.

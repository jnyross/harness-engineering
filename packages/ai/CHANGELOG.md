...
- Google/Gemini usage metadata parsing now also rejects whitespace-padded numeric-string token counters instead of trimming/coalescing malformed usage values.
- Google Gemini CLI / Antigravity endpoint resolution now also rejects whitespace-padded model `baseUrl` values instead of trimming/coalescing malformed endpoint identifiers.
- Bedrock and OpenAI Responses usage parsing now also rejects whitespace-padded numeric-string token counters instead of trimming/coalescing malformed usage values.
...
- Shared abortable sleep now clamps oversized timeout values to Node.js timer limits, preventing implicit runtime timer clamping from resolving oversized waits prematurely.
...
- OpenAI Codex OAuth token exchange/refresh parsing now requires positive safe-integer `expires_in` values (rejecting fractional/unsafe values) in addition to object-root + strict token-string validation.
- OpenAI Codex OAuth token exchange/refresh parsing now also rejects whitespace-padded `access_token`/`refresh_token` fields instead of trimming malformed token values.
...
- Anthropic OAuth token exchange/refresh parsing now requires positive safe-integer `expires_in` values (rejecting fractional/unsafe values) in addition to object-root + strict token-string validation.
- Anthropic OAuth token exchange/refresh parsing now also rejects whitespace-padded `access_token`/`refresh_token` fields instead of trimming malformed token values.
...
- Lazy Node-module helpers in `env-api-keys`, OpenAI Codex Responses, and OpenAI Codex OAuth now use top-level type imports (instead of inline `typeof import("node:*").*` annotations), aligning with static type-import conventions while preserving lazy runtime loading.
- HTTP proxy bootstrap now only initializes undici proxy dispatchers when proxy environment variables are set, and reports import/setup failures with explicit diagnostics instead of unhandled promise rejections.

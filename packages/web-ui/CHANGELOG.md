...
- `SandboxedIframe` now centralizes its iframe base-style reset (including Chrome extension font-size normalization) in a dedicated constant, clarifying why host-injected iframe CSS is overridden.
...
- `ApiKeyPromptDialog` polling now serializes async key checks and catches storage-read failures, preventing overlapping poll requests and unhandled interval rejections when key storage access throws.
...
- `AgentInterface.sendMessage()` now sequence-guards async API-key/prompt hooks across disconnect/session swaps, preventing stale async send flows from mutating detached or replaced sessions.
...
- `ProvidersModelsTab.refreshProvider()` now also suppresses stale async refresh completions after disconnect/remount races, preventing detached status updates and stale error alerts.
...
- `CustomProviderDialog.save()` now suppresses stale detached error alerts and isolates `onSave` callback failures so successful saves still close deterministically.
...
- Model-discovery numeric parsing now also rejects whitespace-padded numeric-string context/token metadata values instead of trimming/coalescing malformed remote metadata.
...
- Model-discovery llama.cpp/vLLM model-id parsing now rejects whitespace-padded IDs instead of trimming/coalescing them, preserving strict discovered-model identifier identity.
